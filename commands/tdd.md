...
3. **Boucle** rouge → vert → refactor. Ce qui compte n'est pas l'ordre mais la **confrontation** : tout test doit avoir été vu échouer face à une implémentation fausse.
   ...
   - `[GREEN]` — implémentation **complète, en une passe**, qui rend tout le lot vert. Pas d'émergence pas à pas : c'est une discipline humaine. La contrainte n'est pas la taille du pas mais que **rien ne dépasse la spec** — aucune ligne qu'aucun test du lot n'exige. Relancer la suite complète.
